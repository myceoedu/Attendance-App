...
-- Idempotent realtime membership (safe if re-run).
DO $pub$
BEGIN
  IF NOT EXISTS (
    ...
    EXECUTE 'ALTER PUBLICATION supabase_realtime ADD TABLE public.expense_claims';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    EXECUTE 'ALTER PUBLICATION supabase_realtime ADD TABLE public.claim_attachments';
  END IF;
END
$pub$;
